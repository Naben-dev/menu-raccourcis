...
AddEventHandler('hospital:client:TakeBrancard', function()
    ...
    if Vehicle ~= 0 then
        ...
        if Obj ~= nil or Obj ~= 0 then
            ...
            ESX.ShowNotification("Something went wrong my friend..")
        end
    else
        ESX.ShowNotification("You are not at an Ambulance vehicle..")
    end
end)
...
AddEventHandler('hospital:client:RemoveBrancard', function()
    ...
    if BrancardObject ~= nil then
        ...
        if Dist < 3.0 then
            ...
            ESX.ShowNotification('You are too far away!')
        end
    end
end)
        ...
        if BrancardObject ~= nil then
            ...
            if Distance <= 1.0 then
                if not IsAttached then
                    DrawText3Ds(OffsetCoords.x, OffsetCoords.y, OffsetCoords.z, '~g~E~w~ - Grab a stretcher / ~g~H~w~ Pinning')
                    ...
                    DrawText3Ds(OffsetCoords.x, OffsetCoords.y, OffsetCoords.z, '~g~E~w~ - Release the stretcher')
                    if IsControlJustPressed(0, 51) then
                        DetachBrancard()
                        IsAttached = false
                    end
                ...
                if not IsLayingOnBed then
                    if not IsAttached then
                        DrawText3Ds(OffsetCoords.x, OffsetCoords.y, OffsetCoords.z + 0.2, '~g~G~w~ - Lie on a stretcher')
            ...
            elseif Distance <= 2 then
                if not IsLayingOnBed then
                    ...
                    if not IsAttached then
                        DrawText3Ds(OffsetCoords.x, OffsetCoords.y, OffsetCoords.z + 0.2, '~g~G~w~ - Step off')
                        if IsControlJustPressed(0, Keys["G"]) or IsDisabledControlJustPressed(0, Keys["G"]) then
                            GetOffBrancard()
                        end
                    end
...
AddEventHandler('qb-radialmenu:client:Result', function(IsBusy, type)
    ...
    if type == "lay" then
        if not IsBusy then
            ...
            ESX.ShowNotification("Deze brancard is al in gebruik!", "error")
            IsLayingOnBed = false
        end
    else
        if not IsBusy then
            ...
            ESX.ShowNotification("This stretcher is already in use!", "error")
            IsAttached = false
        end
    end
end)
